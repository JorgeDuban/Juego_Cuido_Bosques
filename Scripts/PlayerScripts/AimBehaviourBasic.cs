...
{
	public string aimButton = "Aim", shoulderButton = "Aim Shoulder", Fire1Button="Fire1";     // Default aim and switch shoulders buttons.
	...
	private bool fire;												      // Boolean to determine whether or not the player is shooting.
	...
	void Update ()
	{
		...
		if (Input.GetAxisRaw(aimButton) != 0 && !aim)
		{
			...
			if (Input.GetKey(KeyCode.Mouse0) && aim)
			{
				fire = true;
				//StartCoroutine(ToggleAimOn());
		...
		behaviourManager.GetAnim.SetBool(firebool, fire);
	...
	private IEnumerator ToggleAimOff()
	{
		...
		private IEnumerator ToggleFireOn()
	{
	...
	private IEnumerator ToggleFireOff()
	{
		fire = false;
